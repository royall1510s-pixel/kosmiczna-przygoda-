...
data class Puff(
    ...
    inner class SpaceGameView : View(this) {
        ...
        private fun updateColonyProduction(dt: Float) {
            ...
            val crewBonus = if (peopleUnlocked) 1f + habitatLevel * 0.06f else 1f

            if (energy > 0f) {
                iron = min(storageLimit, iron + (0.35f + ironMineLevel * 0.42f) * power * dt * researchBonus)
                water = min(storageLimit, water + (0.22f + powerLevel * 0.05f) * power * dt * crewBonus)
                ice = min(storageLimit, ice + (0.06f + iceExtractorLevel * 0.28f) * power * dt)
                crystal = min(storageLimit, crystal + (0.02f + crystalLabLevel * 0.11f) * power * dt * researchBonus)
                food = min(storageLimit, food + (0.05f + foodFarmLevel * 0.16f) * power * dt * crewBonus)
